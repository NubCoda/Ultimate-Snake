...
public class AppleModel extends Observable implements Actor {
	...
	public AppleModel(GamePanelView gamePanelView) {
		...
		// bufferedImages =
		// ImageIO.read(getClass().getClassLoader().getResourceAsStream("recources/apple_sprite.png"));
		// bounding = new Rectangle(newPosition_x, newPosition_y,
		// bufferedImages.getWidth(), bufferedImages.getHeight());
	...
	public void update() {
		// applePosition_x =
		// Math.random()*(double)(maxWindowsize_x-minWindowsize_x)-(double)minWindowsize_x;
		// applePosition_y =
		// Math.random()*(double)(maxWindowsize_y-minWindowsize_y)-(double)minWindowsize_y;
		applePosition_x = Math.random()
				* (double) (gamePanelView.getWidth() - 0) - (double) 0;
		applePosition_y = Math.random()
				* (double) (gamePanelView.getHeight() - 0) - (double) 0;
		setChanged();
		notifyObservers();
	}
